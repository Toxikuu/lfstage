...
SOURCES="$LFS/sources"
LFSTAGE_SOURCES="/code/lfstage/sources"

download() {
  local url="$1"
  local filename=$(basename "$url")

  if [ ! -e "$LFSTAGE_SOURCES/$filename" ]; then
    echo "Downloading $filename..."
    wget -P "$LFSTAGE_SOURCES" "$url" || return 1
  else
    echo "SKIPPING: $filename already exists in $LFSTAGE_SOURCES"
  fi
}
...
links=(
  ...
  "https://www.kernel.org/pub/linux/kernel/v6.x/linux-6.13.2.tar.xz"
